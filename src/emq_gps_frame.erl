...
-type parser() :: fun((binary()) -> {ok, gps_frame(), binary()} 
                                  | {more, parser()}
                                  | {error, any()}).
...
-spec parse(Phase :: atom(), binary(), #parser_state{}) ->
    {ok, gps_frame(), binary()} | {more, parser()} | {error, any()}.
...
new_frame(#parser_state{cmd = Cmd, headers = Headers, acc = Acc}) ->
    #gps_frame{command = Cmd, headers = Headers, body = Acc}.
...
serialize(#gps_frame{command = Cmd, headers = Headers, body = Body}) ->
...
make(<<"CONNECTED">>, Headers) ->
    #gps_frame{command = <<"CONNECTED">>,
                 headers = [{<<"server">>, ?GPS_SERVER} | Headers]};

make(Command, Headers) ->
    #gps_frame{command = Command, headers = Headers}.

make(Command, Headers, Body) ->
    #gps_frame{command = Command, headers = Headers, body = Body}.
